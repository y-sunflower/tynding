...
`tynding` is an R package that compiles Typst documents from R through a
Rust backend.
...
Multi-page `png` and `svg` exports with multiple pages will create
multiple files. In order to [behaves the same as the Typst
...
You can send inputs thanks to Typst sys inputs handling. Basically, you
do your things with R, and then send whatever you want to Typst! For
example:
...
This means that we can, for instance, send a dataframe from R to create
a Typst table.
...
If you’re coding with AI, this page is pretty much all they need to
know! Just copy everything above this section and send it to your
favorite AI/LLM.
